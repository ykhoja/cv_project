...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, shape = (None, height, width, nchannels * history))
        self.a = tf.placeholder(tf.int32, shape = (None))
        self.r = tf.placeholder(tf.float32, shape = (None))
        self.sp = tf.placeholder(tf.uint8, shape = (None, height, width, nchannels * history))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse) as scope:
            out = layers.fully_connected(inputs = s, num_outputs = num_actions, activation_fn = None, reuse = reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        opAssigns = []
        q_values = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, q_scope)
        target_q_values = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, target_q_scope)
    ...
    def add_loss_op(self, q, target_q):
        ...
        # Create q_samp vector
        max_q = tf.reduce_max(target_q, axis = 1)
        done_mask = tf.to_float(tf.logical_not(self.done_mask))
        q_samp = tf.add(self.r, tf.multiply(tf.constant(self.config.gamma, dtype=tf.float32), tf.multiply(done_mask, max_q)))

        # Get q_actual
        actions = tf.one_hot(self.a, num_actions)
        q_actual = tf.reduce_sum(tf.multiply(q, actions), axis = 1)

        # Find average loss
        self.loss = tf.reduce_mean(tf.square(q_samp - q_actual))
    ...
    def add_optimizer_op(self, scope):
        ...
        pairs = optimizer.compute_gradients(self.loss, var_list = variables)

        # Clip gradients 
        if self.config.grad_clip:
            grads_and_vars = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad, var in pairs if grad is not None]
        else:
            grads_and_vars = pairs
